# ref: https://atcoder.jp/contests/tessoku-book/tasks/tessoku_book_bg
...
class SegTree(object):
    ...
    def set(self, idx, val) -> None:
        ...
        while idx:
            self.tree[idx >> 1] = sum((self.tree[idx], self.tree[idx ^ 1]))
            idx >>= 1
    ...
    def query(self, left, right) -> int:
        ...
        while left < right:
            if left & 1:
                ret = sum((ret, self.tree[left]))
                left += 1
            if right & 1:
                ret = sum((ret, self.tree[right - 1]))
            left >>= 1
            right >>= 1
        return ret
